...
const get_bbox = (label: any, ai_bbox: any) => {
...
const onClickUseFull = ({ bbox_set, label }: BBoxProps): MouseEventHandler<HTMLDivElement> => (e) => {
    ...
    bbox_set(get_bbox(label, ai_bbox));
}

const onChangeBBox = ({ bbox_show_set }: BBoxProps, value: NumberUndefined) => (e: ChangeEvent) => {
    bbox_show_set(value === 1 ? 0 : 1);
}

const onChangeUpdate = ({ bbox_update_set, bbox_show_set }: BBoxProps, value: NumberUndefined) => (e: ChangeEvent) => {
...
const BBox = (props: BBoxProps) => {
    if (props.bbox && Object.keys(props.bbox).length) {
        return (
            <>
                <label>
                    <input type='checkbox' checked={ props.bbox_show === 1 } onChange={ onChangeBBox(props, props.bbox_show) }/> b. 박스 보기
                </label>
                <label>
                    <input type='checkbox' checked={ props.bbox_update === 1 } onChange={ onChangeUpdate(props, props.bbox_update) }/> 박스 수정하기
                </label>
            </>
        )
    } else {
        if (props.label && props.label['이미지 정보'] && props.cloth_type && props.cloth_type === 'upper') {
